...
namespace iPlay
{
	public class _CustomFormOld : Form
	{
		private UI.Container uiForm;
		
		protected UI.OldButton buttonMinimize;
		protected UI.OldButton buttonClose;
		...
		protected _CustomFormOld(int width, int height)
		{
			InitializeComponent();
			...
			this.buttonMinimize = new UI.OldButton(new UI.Rect2D { X = width - 21, Y = 2, W = 9, H = 9 });
			this.buttonClose = new UI.OldButton(new UI.Rect2D { X = width - 11, Y = 2, W = 9, H = 9 });
